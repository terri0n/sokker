...
<head>
	...
	<title>Sokker - Copa Iberia</title>
				...
				<form method="post" action="${pageContext.request.contextPath}/iberia/generar">
					<ul style="list-style: none;">
						<li style="vertical-align: top;">
							...
							<textarea name="equipos" rows="5" cols="50">20263 5661 17908 18104 20900 21063 20848 21034 21117 42456 42481 91262 20341 20436 20847 120789 43256 120330
							</textarea>
						</li>
						<li>	
						...
						<li>	
							...
							<input type="text" size="2" name="equipos_champions" value="4" />
						</li>
						<li>
							<label style="display: inline-block; width: 220px; vertical-align: top">Nº de equipos para la UEFA:</label>
							<input type="text" size="2" name="equipos_uefa" value="4" />
						</li>
						<li>
				...
				<form method="post" action="${pageContext.request.contextPath}/iberia/guardar">
					<c:forEach items="${competicion.grupos}" var="g" varStatus="s">
						...
						<div style="display: inline-block; vertical-align: top">	
							<div class="grupo">
								<h1>Grupo <c:out value="${g.numero}" /></h1>
								<table>
									<thead>
										<tr>
											<th>&nbsp;</th>
									...
									<tbody>
										<c:forEach items="${g.equipos}" var="e" varStatus="var">
											<tr class="equipo c${e.tipo_competicion}${var.count % 2}">
												...
												<td><a href="http://sokker.org/team/teamID/${e.tid}" target="_blank"><c:out value="${e.nombre}" /></a></td>
								...
								<c:forEach items="${g.jornadas}" var="j" varStatus="var">
									<div id="jornada${s.count}_${var.count}" style="${var.count == g.num_jornada?'':'display:none'}">
										<table align="center">
											<thead>
												<tr>
													<th colspan="3"><b><a href="javascript:jornada_click(${s.count}, ${var.count}, -1)">&larr;</a> Jornada ${var.count} / ${g.num_jornadas} <a href="javascript:jornada_click(${s.count}, ${var.count}, 1)">&rarr;</a></b></th>
												</tr>
											</thead>
											...
											<tbody>					
												<c:forEach items="${j.partidos}" var="p" varStatus="var2">
													<tr class="equipo c${var2.count % 2}">
														<td align="right">
															<a href="http://sokker.org/team/teamID/${p.local.tid}" target="_blank"><c:out value="${p.local.nombre}" /></a>
														</td>
														<td>
														...
														<td>
															<a href="http://sokker.org/team/teamID/${p.visitante.tid}" target="_blank"><c:out value="${p.visitante.nombre}" /></a>
														</td>
													</tr>
												</c:forEach>
											</tbody>
										...
										<%-- EQUIPOS --%>
										<br/>
										<textarea onclick="select()" rows="${g.equipos.size()}" cols="50"><c:out value="${g.getTexto_foro(var.count)}" /></textarea>
									</div>
					...
					<c:forEach items="${competicion.competiciones}" var="c">
						<div style="display: inline-block; vertical-align: top">
													...
													<td rowspan="${j.rowspan}" style="padding:0; padding-bottom: 5px;">
														...
														<div class="lineas${varj.index == 0 ? 'J0' : varj.count == c.num_jornadas ? 'JU' : ''} ${(varp.index / varj.count) % 2 == 1 ? 'lineas1 r' : ''}">
															<div class="equipo_comp c${c.tipo_competicion}1">
																<a href="http://sokker.org/team/teamID/${p.local.tid}" target="_blank"><c:out value="${p.local.nombre}" escapeXml="false" /></a>
														...
														<div class="lineas${varj.index == 0 ? 'J0' : varj.count == c.num_jornadas ? 'JU' : ''} ${(varp.index / varj.count) % 2 == 0 ? 'lineas0 r' : ''}">
															<div class="equipo_comp c${c.tipo_competicion}0">
																<a href="http://sokker.org/team/teamID/${p.visitante.tid}" target="_blank"><c:out value="${p.visitante.nombre}" escapeXml="false" /></a>
							...
							<%-- EQUIPOS --%>
							<br/>
							<textarea onclick="select()" rows="${c.equipos.size()}" cols="50"><c:out value="${c.texto_foro}" /></textarea>
						</div>
					</c:forEach>
