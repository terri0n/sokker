...
public class Cambiar_color extends SERVLET_ASISTENTE {
    ...
    protected void execute(HttpServletRequest request, HttpServletResponse response)
        ...
        if (login(request)) {
            ...
            String pidsParam = request.getParameter("pids");
            String color = request.getParameter("color");
            boolean historico = Util.getInt(request.getSession(), "historico") > 0;

            if (pidsParam != null && !pidsParam.trim().isEmpty()
                    && color != null && !color.trim().isEmpty()) {
                String pids = "," + pidsParam;
                if (!pids.endsWith(",")) {
                    pids += ",";
                }
                ...
                List<Jugador> jugadores = AsistenteBO.leer_jugadores(
                        tid, usuario.getDef_equipo(), historico, usuario);
