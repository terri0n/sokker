...
//		List<Jugador> jugadores = new ArrayList<Jugador>();
		JugadorBO.hacer_login_xml(navegador);
//
...
//				boolean seguir;
//				do {
//					System.out.println(pais.getNombre() + " - Página " + pagina + " - " + jugadores.size());
//		
//					HtmlPage html_pagina = navegador.getPage("http://sokker.org/country_ranking/ID_country/" + pais.getId() + "/action/ranking/pg/" + pagina);
...
//		request.setAttribute("jugadores", jugadores);
	...
	public static void scan(HttpServletRequest request, JspWriter out) throws FailingHttpStatusCodeException, MalformedURLException, IOException {
		new Navegador(true) {
			@Override
			protected void execute(WebClient navegador) throws FailingHttpStatusCodeException, MalformedURLException, IOException {
				boolean todos = "".equals(request.getParameter("id_country_filtro"));
				...
				for (Pais pais : paises) {
					if (todos || pais.getId() == id_country_filtro) {
						...
						do {
							...
							for (HtmlAnchor e : lista) {
								Integer tid = Util.stringToInteger(e.getHrefAttribute().split("/")[2]);
								ArrayList<Jugador> busqueda = JugadorBO.obtener_jugadores(tid, id_country_origen, valor, edad, navegador);
								...
								for (Jugador j : busqueda) {
									out.print(
										...
										"	<td><a href=\"http://sokker.org/player/PID/" + j.getPid() + "\">" + j.getNombre() + "</a></td>" + 
										"	<td>" + j.getEdad() + "</td>" + 
