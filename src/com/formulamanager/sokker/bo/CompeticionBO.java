...
public class CompeticionBO {
	...
	public static void buscar_arcades(Competicion competicion) throws FailingHttpStatusCodeException, MalformedURLException, IOException, ParseException {
		new Navegador(true) {
			@Override
				...
				for (Grupo g : todos) {
					...
					for (Partido p : j.getPartidos()) {
						if (p.getGoles_l() == null && p.getGoles_v() == null) {
							XmlPage pagina = navegador.getPage("http://sokker.org/xml/matches-team-" + p.getLocal().getTid() + ".xml");
