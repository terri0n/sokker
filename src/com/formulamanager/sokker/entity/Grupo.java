...
public class Grupo implements Comparable<Grupo> {
	...
	public String getNombre() {
		return tipo_competicion == null ? "GRUPO " + numero : tipo_competicion.getNombre() + "";
	}
	
	public String getTexto_foro(int jornada) {
		String res = "[b]" + getNombre() + "[/b] (Jornada " + jornada + ")\n\n";

		if (getNum_jornada() > 0) {
			Jornada j = jornadas.get(jornada - 1);
			
			for (Partido p : j.getPartidos()) {
				res += "[tid=" + p.getLocal().getTid() + "]" + p.getLocal().getNombre() + "[/tid] - " +
						"[tid=" + p.getVisitante().getTid() + "]" + p.getVisitante().getNombre() + "[/tid] - " +
